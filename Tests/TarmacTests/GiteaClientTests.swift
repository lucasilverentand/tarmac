...
struct GiteaClientTests {
    ...
    func requestConstruction() async throws {
        ...
        let result: Version = try await makeClient().request(
            method: "GET", path: "/version", body: nil, token: "secret", timeoutInterval: 5
        )
    ...
    func authenticationError() async {
        MockURLProtocol.reset()
        MockURLProtocol.addHandler(forPathContaining: "/api/v1/version", statusCode: 401, responseData: Data("denied".utf8))

        await #expect(throws: GiteaAPIError.self) {
            let _: Version = try await makeClient().request(
                method: "GET", path: "/version", body: nil, token: "bad", timeoutInterval: 5
            )
        }
    }
    ...
    func malformedResponse() async {
        ...
        await #expect(throws: GiteaAPIError.self) {
            let _: Version = try await makeClient().request(
                method: "GET", path: "/version", body: nil, token: "token", timeoutInterval: 5
            )
        }
    }
